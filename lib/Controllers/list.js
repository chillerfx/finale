...
List.prototype.fetch = function(req, res, context) {
  ...
  /**
  * Verry poor perfomance when counting on huge SQLite3 tables (5GB+)
  * Just a wokaround
  */
  return model
    .findAll(options)
    .then(function(result) {
      context.instance = result.length;
      var start = offset;
      ...
      if (!!self.resource.pagination)
        res.header('Content-Range', 'items ' + [[start, end].join('-'), result.length].join('/'));
